...
namespace rocksdb {
    namespace titandb {
        ...
        void TitanTableBuilder::Add(const Slice &key, const Slice &value) {
            if (!ok())
                return;
            // 似乎所谓的InternalKey会在userkey的结尾加上8位的额外数据，然后ParseInternalKey简单来说应该是用来剥离这8字节的数据，并形成其他的相关信息，比如type以及seq
            // user_key + sequence[7] + type[1]
            ...
            if (ikey.type == kTypeBlobIndex &&
                ...
                // 按照`TitanBlobRunMode`中的解释，如果value的类型是一个BlobIndex，而且当前的运行模式是Fallback，那么titan就需要从blob文件中读取value并填充到sst文件中，就像是一般的lsm一样
        ...
        // 具体化到添加blob的情况，尽心处理，调用了file_builder的add方法以及base_table的add方法，分别用于
        // 构建blob文件以及SST文件
        ...
        // 统一了blob record以及普通记录的添加操作，通过ctx->has_value来判断是否是blob index
        // 如果由value那么就直接写入value，如果没有value就将blob记录包装为internal_key::blob_index然后再进行加入
