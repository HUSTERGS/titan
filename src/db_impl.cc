...
namespace rocksdb {
    namespace titandb {
        ...
        public:
            ...
            // 创建一个新的blob文件并将相关信息保存在handle当中
            // 新创建的文件编号会被加入到pending_outputs成员中
            ...
            // 对输出的文件进行最后的收尾，Close对应的文件
            // 输出的文件是经过新建操作才获得的，所以需要在pending_outputs中去掉对应的文件
            //
            ...
            // 直接在文件系统中删除对应的文件，而不涉及到blob_file_set
            // 删除的文件将会在pending_outputs中被删除
        ...
        // 通过rocksdb在util中提供的RepeattableThread方法来每隔一段时间持续的调用某一个函数
        ...
        // 检查每一个cf中的option字段
        // 如果开启了level_merge那么必然要开启level_compaction_dynamic_level_bytes，否则报错
        ...
        // 2. 创建log
        // 3. 锁上锁文件
        // 4. 创建blob_file_set，用于管理blob文件
        // 5. 加入event_listener
        // 6. 初始化ColumnFamilyDescriptor
        //      1. merge_operator
        //      2. compaction_filter
        //      3. compaction_filter_factory
        //      4. table_properties_collector_factories
        //      5. disable_auto_compactions
        //      （也就是上述所说的比较重要的cf配置）
        // 7. 初始化gc线程池
        // 8. open base db(也就是rocksdb)，从而填充db_以及handles
        // 9. 通过遍历handles来填充cf_info_成员
        //      - 对于handles中对应的descs中的options，titan人为地拆分为了两个CFOptions，分别是ImmutableTitanOptions或者是MutableTitanOptions
        //      - 单独取出那些option中设定了启用自动压缩(auto compactions)的cf作为cf_with_compaction
        //      - 保存每一个cf对应的ID到descs[i]的映射，作为column_families
        // 10. 调用blob_file_set::Open(column_families)
        //      创建对应的BlobStorage以及其所需的BlobFileCache，再构建cf_id到blob_storage的映射保存在BlobFileSet中的column_families成员中，然后还在stats_成员中初始化了cf对应的信息？（这个部分还不是很懂）
        //      - `AddColumnFamilies`
        //      `stats_->InitializeCF(cf.first, blob_storage)`
        // 11. 调用`InitializeGC(*handles)`方法，基本上相当于进行一次gc
        //      - 对每一个cf分别调用`GetPropertiesOfAllTables(cf_handle, &collection)`。在SST文件中key-value中的value会有两种情况。一种是普通的value，一种是指向blob文件的blob index，分别计算这两种的数据的大小，并分配到不同的blob文件的统计数据中，有点像那个map reduce，最终可以得到每一个blob文件的数据大小，并重新计算每一个文件的gc score。
        //      - 调用blob_storage->InitializeAllFiles()函数，将会把所有的blob文件的状态设置为kRestart，并会调用ComputeGCScore函数，用于计算gc分数。
        //      - 将每一个cf_id都加入到gc队列中
        //      - 调用`MaybeScheduleGC`
        //          如果有还有没有规划的gc任务（通过查看`unscheduled_gc_`来判断），就不断的向线程池提交`BGWorkGC`任务（实际调用了`BackgroundCallGC`函数），但其总数不能超过设置中的`db_options_.max_background_gc`
        //      - `BackgroundCallGC`
        // 12. 对于`9`中遇到的，需要进行自动压缩的cf，进行相应的设置(`db_->EnableAutoCompaction(cf_with_compaction)`)
        // 13. 开启后台调用线程
        //
        Status TitanDBImpl::OpenImpl(const std::vector<TitanCFDescriptor> &descs,
            ...
            // log.
            // 因为info log 是在`CreateLoggerFromOptions`函数调用之后进行初始化的，所以对于`BlobFileSet`的创建
            // 不是在构造函数，而是在这里，是为了能够正常使用log
            // QUES: 那为啥不都放在构造函数
            ...
            // descs本来是TitanCFOptions，继承了ColumnFamilyDescriptor
            for (auto &desc : descs) {
            ...
            // 通过调用基类的DB::Open方法，db_以及handles得到填充
            assert(db_ != nullptr);
            ...
            // 创建对应的BlobStorage以及其所需的BlobFileCache，再构建cf_id到blob_storage的映射，
            // 保存在BlobFileSet中的column_families成员中，然后还在stats_成员中初始化了cf对应的信息？（这个部分还不是很懂）
        ...
        // 以下操作先检查是否有后台进程的error，如果有的话就直接返回错误本身，如果没有的话就调用对应方法的实现，然后返回status
        ...
        Status TitanDBImpl::Get(const ReadOptions &options, ColumnFamilyHandle *handle,
            ...
            // 如果配置中已经指定了snapshot，那么就直接按照指定的snapshot来进行操作，如果没有就获得当前的快照并进行读取
            if (options.snapshot) {
        ...
        Status TitanDBImpl::GetImpl(const ReadOptions &options,
            ...
            // 如果拿到的值有问题或者，拿到的值没有问题，并且是一个值而不是blobindex，那么就直接返回
            ...
            mutex_.Unlock();
            // 如果cf对应的blob_storage存在，那么就将blobindex得到真正的值
            if (storage) {
            ...
            if (s.ok()) {
                value->Reset();
                // 通过Pinnable Slice将record中的value保存在参数value中
